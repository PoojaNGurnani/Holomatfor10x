...
def create_circles():
    ...
    shift_left = 0
    shift_up= 180
    main_circle = AppCircle((center_x-shift_left, center_y-shift_up), main_circle_radius, 0, (center_x, center_y), is_main=True)
    circles.append(main_circle)

    angle_step = 360 / num_circles
    for i in range(num_circles):
        angle = math.radians(angle_step * i)
        x = center_x + int(distance * math.cos(angle))-shift_left
        y = center_y + int(distance * math.sin(angle)) -shift_up
        circles.append(AppCircle((center_x-shift_left, center_y- shift_up), app_circle_radius, i + 1, (x, y)))
    return circles
'''
    ...
    while running:
        ...
        if transformed_landmarks:
            for transformed_coords in transformed_landmarks:
                ...
                #screen_x = int(index_finger_tip[1] * SCREEN_SIZE[0])
                #screen_y = SCREEN_SIZE[0] - int(index_finger_tip[0] * SCREEN_SIZE[1])
                #index_finger_pos = (screen_x, screen_y)
                # Invert the coordinates for Pygame screen
                #screen_x = int((1 - index_finger_tip[1]) * SCREEN_SIZE[0])
                #screen_y = int(index_finger_tip[0] * SCREEN_SIZE[1])
                #index_finger_pos = (screen_x, screen_y)
                #screen_x = SCREEN_SIZE[1] - int(index_finger_tip[1])
                #screen_y = SCREEN_SIZE[0] - int(index_finger_tip[0])
                ...
                #screen_x = int(index_finger_tip[1])
                #screen_y = int(index_finger_tip[0])
                #index_finger_pos = (screen_x, screen_y)
                print(f"Index finger tip transformed coordinates: {index_finger_tip}")
                print(f"Index finger tip screen coordinates: {index_finger_pos}")
        ...
        #pygame.display.get_surface().blit(rotated_screen, (0, 0))
...
def run_home_screen(screen, camera_manager):
    ...
    index_finger_pos = None
    play_sound("./audio/startup.wav")
    calibration_offset_x = 80  # Adjust this value as needed
    calibration_offset_y = -55  # Adjust this value as needed
    ...
    while running:
        ...
        if transformed_landmarks:
            for transformed_coords in transformed_landmarks:
                ...
                # Ensure the coordinates are normalized
                #index_finger_tip[0] = index_finger_tip[0] / SCREEN_SIZE[0]
                #index_finger_tip[1] = index_finger_tip[1] / SCREEN_SIZE[1]

                # Rotate 90 degrees clockwise
                #screen_x = int(index_finger_tip[1] * SCREEN_SIZE[1])
                #screen_y = SCREEN_SIZE[0] - int(index_finger_tip[0] * SCREEN_SIZE[0])
                #index_finger_pos = (screen_x, screen_y)
                ...
                # Rotate 90 degrees clockwise
                screen_x = int(index_finger_tip[1] * SCREEN_SIZE[1]) + calibration_offset_x
                screen_y = SCREEN_SIZE[0] - int(index_finger_tip[0] * SCREEN_SIZE[0]) + calibration_offset_y
                index_finger_pos = (screen_x, screen_y)
...
if __name__ == '__main__':
    ...
    camera_manager = CameraManager('./M.npy', 1080, 1200)
    run_home_screen(screen, camera_manager)
    '''
    os.environ['SDL_VIDEO_WINDOW_POS'] = '-3440,0'
    screen = pygame.display.set_mode((SCREEN_SIZE[1], SCREEN_SIZE[0]), pygame.FULLSCREEN)
    pygame.display.set_caption('Home Screen')
    camera_manager = CameraManager('./M.npy', SCREEN_SIZE[0], SCREEN_SIZE[1])
    run_home_screen(screen, camera_manager)
